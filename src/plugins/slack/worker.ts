...
//
// Outbound: updateTicket detects status transitions and posts into the
// mapped thread (notifySlackThread) — best-effort, silent when Slack
// isn't configured.
//
...
import type { Env, TicketRecord } from "../../types";
 ...
 * (if any). Called from updateTicket on status transitions.
 */
export async function notifySlackThread(env: Env, ticketId: string, text: string): Promise<void> {
  try {
    if (!env.SLACK_BOT_TOKEN) return;
...
async function processEvent(env: Env, e: SlackEvent, selfOrigin: string): Promise<void> {
  ...
  if (mapped) {
    ...
    if (active.includes(rec.status)) {
      ...
      await appendSteer(env, mapped, text.slice(0, 4000));
  ...
  const r = await runFleetChat(env, selfOrigin, [{ role: "user", content: text }]);
...
export const slackPlugin: SourcePlugin = {
  id: "slack",
  ...
  // Unused (handle takes over), but the interface requires it.
  async parse() {
    return [];
  },

  async handle(request, payload, env, ctx) {
    ...
    ctx.waitUntil(processEvent(env, e, selfOrigin));
