...
import { getAgentStateConfig } from "../../lib/theme/status.ts";
import { colors } from "../../lib/theme/colors.ts";
import { readSessionMemory, hasSessionMemory } from "../../harness/session/session-memory.ts";
...
export function useAgentProgress(
  ...
  const refresh = () => {
    try {
      ...
      if (worktreePath) {
        hasMemory = hasSessionMemory(worktreePath);
        if (hasMemory) {
          sessionMemory = readSessionMemory(worktreePath);
        }
      }
