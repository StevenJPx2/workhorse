...
import { updateTicket } from "../../lib/db/index.ts";
import { insertTicketEvent } from "../../lib/db/events.ts";
import type {
  UseJiraSyncOptions,
  UseJiraSyncReturn,
  JiraSyncStatus,
...
function getTransitionId(status: string): string | undefined {
...
export function useJiraSync(options: UseJiraSyncOptions = {}): UseJiraSyncReturn {
  ...
  async function withSyncLock<T>(
    ...
    try {
      ...
      recordSyncEvent(ticketId, action, `Success at ${now}`);
      ...
      recordSyncEvent(ticketId, action, `Failed: ${error.message}`);
      throw error;
    } finally {
      setIsSyncing(false);
    }
