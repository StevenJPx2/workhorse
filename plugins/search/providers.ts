// Web-search provider chain (shared by the /search + /read routes and the
// web_search + web_read stage tools).
//
...
// Keys live worker-side. Provider docs:
...
const jina: Provider = async (env, query, count) => {
  ...
  const r = await fetch(`https://s.jina.ai/?q=${encodeURIComponent(query)}&num=${count}`, {
    headers: {
      ...
      "x-respond-with": "no-content", // snippets only — full pages via web_read
    },
  });
  if (!r.ok) return null;
