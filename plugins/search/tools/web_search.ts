...
export default tool({
  ...
  docs: `
web_search — search the live web. Provider API keys stay worker-side and never
enter the container.
...
OUTPUT
  Numbered { title, url, snippet }.

EXAMPLES

  { query: "valibot discriminated union schema" }
  { query: "TS2741 property missing", count: 5 }

NOTES
  Triage from the SNIPPETS and web_read only the promising hits — reading every
  result is expensive and usually unnecessary.
