...
Captures timed frames and assembles them with ffmpeg. GIF rather than WebM
because GitHub renders a GIF inline in a PR description.

ARGUMENTS
  savePath    (required) destination .gif
  durationMs  default 6000, capped at 12000
  fps         default 2, clamped to 1-4
  script      optional JS to run before capture (e.g. a scroll or a click)

Needs at least 2 frames, so keep durationMs above one frame interval.

EXAMPLES

  { savePath: "/tmp/demo.gif" }
  { savePath: "/tmp/scroll.gif", durationMs: 4000, script: "window.scrollTo(0,600)" }

NOTES
  Use this for a FLOW (a scroll, a transition, an interaction); a single state is
  better served by browser_screenshot. Upload with upload_image to embed it.
`,
  input: v.object({
    savePath: v.string(),
