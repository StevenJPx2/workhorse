// Stage tools: text/code hosting (flue engine).
//
// The flue port of extension.ts: the agent loop runs in the Worker, so
// this is a PluginToolFactory the stage assembles rather than a
// sandbox-scanned Pi extension. `upload_text` hosts text/code on a keyless
// paste host and returns a RAW curl-able URL. `path` mode reads the file
// through the sandbox handle (the file lives in the container, the tool
// runs worker-side).
//
// Host chain (first that VERIFIABLY serves the bytes back wins — a host can
// accept an upload yet serve nothing, so we confirm, we don't trust):
...
export const pasteTools: PluginToolFactory = ({ sandbox }) => [
  defineTool({
    name: "upload_text",
    description:
