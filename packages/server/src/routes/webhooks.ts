...
import { appendEvents, wakeTicket } from "@workhorse/events";
...
export const webhookRoutes: Route[] = [
  {
    ...
    auth: "public",
    async handler({ request, env, ctx, url, match, core, pluginFor }) {
      ...
      if (webhook.handle) {
        return webhook.handle(request, payload, env, ctx, core(env, url.origin));
      }
