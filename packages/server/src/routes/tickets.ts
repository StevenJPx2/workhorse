...
/**
 * The latest round's analysis for a stage, read from the sandbox.
 *
 * submit_work writes analysis.md at stage completion, so the newest round dir is
 * the current or last output. Tail-capped: a long analysis is truncated from the
 * FRONT, because the conclusion is at the end.
 */
async function stageAnalysis(env: Env, ticketId: string, runId: string, stageId: string): Promise<string | null> {
  const { sandboxDriver } = await import("@workhorse/sandbox");
  const driver = sandboxDriver(env, `ticket-${ticketId}`);

  const listed = await driver.exec(
    `ls -1dt /workspace/.workflow/${runId}/stages/${stageId}/round-* 2>/dev/null | head -1`,
    { timeout: 15_000 },
  );

  const dir = listed.stdout?.trim().split("\n")[0];
  if (!dir) return null;

  return (await driver.readFile(`${dir}/analysis.md`))?.slice(-12_000) ?? null;
}
...
export const ticketRoutes: Route[] = [
  {
    ...
    path: "/tickets",
    auth: "master",
    async handler({ request, env, url, intake }) {
      const body = (await request.json()) as Partial<TicketParams>;
      const r = await intake.fileTicket(env, { ...body, selfOrigin: url.origin });
  ...
  },
  {
    ...
    auth: "master",
    async handler({ request, env, ctx, url, match, core, intake }) {
      const rec = await db(env).tickets.get(match[1]);
      if (!rec) return json({ error: "not found" }, 404);
  ...
  },
  {
    ...
    async handler({ env, match }) {
      ...
      try {
        // Flue-first: the stage session runs in the Worker, so there is no on-disk
        // event stream. The live: snapshot is the signal; the stage's analysis.md
        // is the readable output.
        const live = await env.TICKETS.get(`live:${match[1]}`);
        ...
        return json({
          stage: stageId,
          status: snap.note ?? null,
          output: stageId ? await stageAnalysis(env, match[1], rec.runId, stageId) : null,
        });
      } catch (e) {
