...
import type { FileTicketResult, Intake } from "@workhorse/intake";
...
export async function fireTrigger(
  ...
  const r = await intake.fileTicket(env, {
    repo: t.repo,
    prompt,
 ...
 */
/**
 * The most recent minute in the sweep window that this schedule matches and that
 * is newer than its last firing, or null.
 *
 * Sweeps run every 15 minutes over a 16-minute window, so consecutive sweeps
 * overlap deliberately — a run that fails to start is retried next time rather
 * than skipped. The `<= last` bound is what stops the overlap double-firing.
 */
function dueMinute(schedule: string, lastFiredAt: string | undefined, now: number, windowMinutes: number): Date | null {
  const last = lastFiredAt ? new Date(lastFiredAt).getTime() : 0;

  for (let m = 0; m < windowMinutes; m++) {
    const at = new Date(now - m * 60_000);
    if (at.getTime() <= last) return null;
...
export async function sweepCronTriggers(intake: Intake, env: Env, windowMinutes = 16): Promise<string[]> {
  const fired: string[] = [];
  const now = Date.now();
  ...
  for (const t of await listTriggers(env)) {
    ...
    const at = dueMinute(t.schedule, t.lastFiredAt, now, windowMinutes);
    if (!at) continue;

    const r = await fireTrigger(intake, env, t.name, {
      input: `Scheduled run (${t.schedule}) at ${at.toISOString()}`,
      date: at.toISOString(),
    });
    // The ticket id (or the error) rather than just the name, so a log line says
