...
import type { Core, ToolContext, WritePolicy } from "@workhorse/api";
import { sandboxDriver } from "./driver";

/**
 * What the bridge needs from the composition root.
 *
 * Both reach into the plugin registry, which is why they are injected: importing
 * it here would make @workhorse/sandbox depend on every plugin.
 */
export interface BridgeDeps {
  /** (agent ∪ services) ∩ stage-allowlist — the capability gate. */
  assembleStageTools: (ctx: ToolContext, allow: readonly string[]) => WorkhorseTool[];
  /** The Core facade a tool sees. */
  coreFor: (env: Env, selfOrigin: string) => Core;
}

/** Build the ToolContext a bridged tool call runs in. */
function bridgeContext(
  ...
  return {
    env,
    core: deps.coreFor(env, p.selfOrigin),
    selfOrigin: p.selfOrigin,
    sandbox: sandboxDriver(env, p.sandboxId),
    ticket: { id: p.ticketId, repo: p.repo, stage: p.stage },
    policy,
  };
}
 ...
 */
function stageTools(env: Env, deps: BridgeDeps, p: ToolBridgeProps): Map<string, WorkhorseTool> {
  const ctx = bridgeContext(env, deps, p, { dir: p.dir, writeAllow: p.writeAllow });
  const map = new Map<string, WorkhorseTool>();
  for (const t of deps.assembleStageTools(ctx, p.allow)) map.set(t.name, t);
  return map;
}
 ...
 */
export function makeToolBridge(deps: BridgeDeps) {
  return class ToolBridge extends WorkerEntrypoint<Env, ToolBridgeProps> {
  ...
  async invoke(name: string, input: unknown): Promise<unknown> {
    ...
    const tool = stageTools(this.env, deps, p).get(name);
