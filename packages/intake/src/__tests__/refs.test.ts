...
const providers = new Map<string, AttachmentProvider>([
  [
    "jira",
    {
      kind: "jira",
      label: "Jira",
      match: (s) => /^[A-Z]+-\d+$/.exec(s)?.[0] ?? null,
      resolve: async () => ({ title: "PROJ-42", content: "body" }),
    },
  ],
  [
    "slack",
    {
      ...
      resolve: async () => ({ title: "thread", content: "body" }),
    },
  ],
]);

import { parseRefs as parse, rankedRefs, recordRefUse } from "../refs";

/** parseRefs with this suite's providers bound. */
const parseRefs = (input: string) => parse(providers, input);
...
describe("parseRefs", () => {
  ...
  it("survives a provider whose match() throws", () => {
    providers.set("broken", {
      ...
      resolve: async () => ({ title: "never reached", content: "" }),
    });
