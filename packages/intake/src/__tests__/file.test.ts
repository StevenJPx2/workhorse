...
vi.mock("@workhorse/db", async (importOriginal) => ({
  ...
  db: () => ({ tickets: { put: putTicket } }),
}));
vi.mock("@workhorse/auth", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@workhorse/auth")>()),
  modelToken: () => ({ usable }),
}));
vi.mock("../refs", () => ({
  // Providers come FIRST now (they are injected, not imported), so the stand-in
  // takes and ignores them. Minimal plugin-driven parser: JIRA-style keys only,
  // which is enough to exercise the enrichment branch.
  parseRefs: (_providers: unknown, input: string) =>
...
const providers = new Map([
  [
    "jira",
    { kind: "jira", label: "Jira", match: () => null, resolve: async () => ({ title: "PROJ-42", content: "body" }) },
  ],
]) as never;

const { fileTicket: file } = await import("../file");

/** fileTicket with this suite's providers bound. */
const fileTicket = (env: Parameters<typeof file>[1], body: Parameters<typeof file>[2]) => file(providers, env, body);
