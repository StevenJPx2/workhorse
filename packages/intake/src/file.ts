...
//
// Attachment providers arrive as a parameter, not an import: reaching into the
// plugin registry from here would make this package depend on every plugin. The
// composition root binds them once via createIntake().

import { db } from "@workhorse/db";
import type { AttachmentProvider, Core, Env, TicketParams, TicketRecord } from "@workhorse/api";
import { START_RUNWAY_MS, modelToken } from "@workhorse/auth";
import { parseRefs, recordRefUse } from "./refs";

/** Attachment providers contributed by plugins, keyed by kind. */
type Providers = Map<string, AttachmentProvider>;
 ...
 */
function enrichableRefs(
  providers: Providers,
  prompt: string,
  attachments: Array<{ kind: string; ref: string }> = [],
): EnrichableRef[] {
  const all: EnrichableRef[] = [
    ...parseRefs(providers, prompt).filter((r) => r.kind !== "repo"),
 ...
 */
async function withEnrichableContext(
  providers: Providers,
  env: Env,
  prompt: string,
  attachments?: Array<{ kind: string; ref: string }>,
) {
  const enrichable = enrichableRefs(providers, prompt, attachments);
...
export async function fileTicket(
  ...
  body.prompt = await withEnrichableContext(providers, env, body.prompt, body.attachments);
