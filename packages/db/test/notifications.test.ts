...
describe("sequence allocation", () => {
  it("starts at 1 for a ticket's first notification", async () => {
    const n = await db.queueNotification({ ticketId: "t1", source: "ui", body: "hello" });
    expect(n.seq).toBe(1);
  });

  it("increments per ticket", async () => {
    await db.queueNotification({ ticketId: "t1", source: "ui", body: "one" });
    await db.queueNotification({ ticketId: "t1", source: "ui", body: "two" });
    const third = await db.queueNotification({ ticketId: "t1", source: "ui", body: "three" });

    expect(third.seq).toBe(3);
  });

  it("counts sequences independently per ticket", async () => {
    await db.queueNotification({ ticketId: "t1", source: "ui", body: "a" });
    await db.queueNotification({ ticketId: "t1", source: "ui", body: "b" });
    const other = await db.queueNotification({ ticketId: "t2", source: "ui", body: "c" });
  ...
  it("survives concurrent queueing without a primary-key collision", async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 8 }, (_, i) => db.queueNotification({ ticketId: "t1", source: "ui", body: `n${i}` })),
    );
...
describe("fields", () => {
  it("returns urgent as a boolean, not 0/1", async () => {
    const n = await db.queueNotification({ ticketId: "t1", source: "ui", body: "x", urgent: true });
    ...
    expect((await db.unreadNotifications("t1"))[0].urgent).toBe(true);
  });

  it("defaults urgent to false and kind to comment", async () => {
    const n = await db.queueNotification({ ticketId: "t1", source: "ui", body: "x" });
  ...
  it("truncates the body at 8000 chars", async () => {
    const n = await db.queueNotification({ ticketId: "t1", source: "ui", body: "x".repeat(9000) });
...
describe("read receipts", () => {
  beforeEach(async () => {
    for (const body of ["one", "two", "three"]) {
      await db.queueNotification({ ticketId: "t1", source: "ui", body });
    }
  });

  it("returns everything unread, oldest first", async () => {
    expect((await db.unreadNotifications("t1")).map((n) => n.body)).toEqual(["one", "two", "three"]);
  });

  it("marks up to a seq and leaves later ones unread", async () => {
    await db.markNotificationsRead("t1", 2);
    expect((await db.unreadNotifications("t1")).map((n) => n.body)).toEqual(["three"]);
  });

  it("does not overwrite an existing receipt", async () => {
    await db.markNotificationsRead("t1", 1);
    const first = (await db.listNotifications("t1")).find((n) => n.seq === 1);

    await db.markNotificationsRead("t1", 3);
    const again = (await db.listNotifications("t1")).find((n) => n.seq === 1);
  ...
  it("leaves another ticket's queue untouched", async () => {
    await db.queueNotification({ ticketId: "t2", source: "ui", body: "other" });
    await db.markNotificationsRead("t1", 3);

    expect(await db.unreadNotifications("t2")).toHaveLength(1);
  });
});
...
describe("listNotifications", () => {
  it("returns newest first, including read ones", async () => {
    ...
    const all = await db.listNotifications("t1");
  ...
  it("honours the limit", async () => {
    for (let i = 0; i < 5; i++) {
      await db.queueNotification({ ticketId: "t1", source: "ui", body: `n${i}` });
    }
    expect(await db.listNotifications("t1", 2)).toHaveLength(2);
  });
});
